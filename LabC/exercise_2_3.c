...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    {
        int64_t *arr = calloc(sizeof(int64_t), futhark_shape_i64_1d(ctx,
                                                                    result_0)[0]);
        
        assert(arr != NULL);
        assert(futhark_values_i64_1d(ctx, result_0, arr) == 0);
        write_array(stdout, binary_output, &i64_info, arr,
                    futhark_shape_i64_1d(ctx, result_0), 1);
        free(arr);
    }
    printf("\n");
    assert(futhark_free_i64_1d(ctx, result_0) == 0);
...
static int futrts_main(struct futhark_context *ctx,
                       struct memblock *out_mem_p_4508,
                       struct memblock is_mem_4483, struct memblock as_mem_4484,
                       int64_t implz2081U_4461);
                       ...
                       int64_t implz2081U_4461)
{
    ...
    int err = 0;
    size_t mem_4486_cached_sizze_4509 = 0;
    char *mem_4486 = NULL;
    struct memblock out_mem_4504;
    
    out_mem_4504.references = NULL;
    if (mem_4486_cached_sizze_4509 < (size_t) (int64_t) 80) {
        mem_4486 = realloc(mem_4486, (int64_t) 80);
        mem_4486_cached_sizze_4509 = (int64_t) 80;
    }
    for (int64_t i_4505 = 0; i_4505 < (int64_t) 10; i_4505++) {
        int64_t x_4506 = (int64_t) 0 + i_4505 * (int64_t) 1;
        
        ((int64_t *) mem_4486)[i_4505] = x_4506;
    }
    for (int64_t write_iter_4473 = 0; write_iter_4473 < implz2081U_4461;
         write_iter_4473++) {
        int64_t write_iv_4475 = ((int64_t *) is_mem_4483.mem)[write_iter_4473];
        int64_t write_iv_4476 = ((int64_t *) as_mem_4484.mem)[write_iter_4473];
        bool x_4468 = sle64((int64_t) 0, write_iv_4475);
        bool y_4469 = slt64(write_iv_4475, (int64_t) 10);
        bool bounds_check_4470 = x_4468 && y_4469;
        bool index_certs_4471;
        
        if (!bounds_check_4470) {
            ctx->error = msgprintf("Error: %s%lld%s%lld%s\n\nBacktrace:\n%s",
                                   "Index [", write_iv_4475,
                                   "] out of bounds for array of shape [",
                                   (int64_t) 10, "].",
                                   "-> #0  exercise_2_3.fut:10:25-31\n   #1  exercise_2_3.fut:10:14-35\n   #2  exercise_2_3.fut:16:5-41\n   #3  exercise_2_3.fut:15:1-16:41\n");
            if (memblock_unref(ctx, &out_mem_4504, "out_mem_4504") != 0)
                return 1;
            return 1;
        }
        
        int64_t defunc_1_f_res_4472 = add64(write_iv_4475, write_iv_4476);
        bool less_than_zzero_4477 = slt64(write_iv_4475, (int64_t) 0);
        bool greater_than_sizze_4478 = sle64((int64_t) 10, write_iv_4475);
        bool outside_bounds_dim_4479 = less_than_zzero_4477 ||
             greater_than_sizze_4478;
        
        if (!outside_bounds_dim_4479) {
            ((int64_t *) mem_4486)[write_iv_4475] = defunc_1_f_res_4472;
        }
    }
    
    struct memblock mem_4502;
    
    mem_4502.references = NULL;
    if (memblock_alloc(ctx, &mem_4502, (int64_t) 80, "mem_4502")) {
        err = 1;
        goto cleanup;
    }
    memmove(mem_4502.mem + (int64_t) 0, mem_4486 + (int64_t) 0, (int64_t) 10 *
            (int64_t) sizeof(int64_t));
    if (memblock_set(ctx, &out_mem_4504, &mem_4502, "mem_4502") != 0)
        return 1;
    (*out_mem_p_4508).references = NULL;
    if (memblock_set(ctx, &*out_mem_p_4508, &out_mem_4504, "out_mem_4504") != 0)
        return 1;
    if (memblock_unref(ctx, &mem_4502, "mem_4502") != 0)
        return 1;
    if (memblock_unref(ctx, &out_mem_4504, "out_mem_4504") != 0)
        return 1;
...
int futhark_entry_main(struct futhark_context *ctx,
                       ...
                       struct futhark_i64_1d *in1)
{
    struct memblock is_mem_4483;
    
    is_mem_4483.references = NULL;
    
    struct memblock as_mem_4484;
    
    as_mem_4484.references = NULL;
    ...
    lock_lock(&ctx->lock);
    is_mem_4483 = in0->mem;
    implz2081U_4461 = in0->shape[0];
    as_mem_4484 = in1->mem;
    implz2081U_4461 = in1->shape[0];
    if (!(implz2081U_4461 == in0->shape[0] && implz2081U_4461 ==
          in1->shape[0])) {
        ret = 1;
        if (!ctx->error)
            ctx->error =
        ...
        ret = futrts_main(ctx, &out_mem_4504, is_mem_4483, as_mem_4484,
                          implz2081U_4461);
        if (ret == 0) {
            assert((*out0 =
                    (struct futhark_i64_1d *) malloc(sizeof(struct futhark_i64_1d))) !=
                NULL);
            (*out0)->mem = out_mem_4504;
            (*out0)->shape[0] = 10;
        }
    }
    lock_unlock(&ctx->lock);
